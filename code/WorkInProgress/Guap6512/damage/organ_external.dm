...
/datum/organ/external
	...
	//here go тканевая хуйня ёпта

	var/skin_stat = 1				//1 means everything is OK, 2 - damaged, 3 - torn.
	var/muscle_stat = 1
	var/bone_stat = 1
...
/datum/organ/external/proc/update_wounds()
	...
	for(var/datum/wound/W in wounds)
		...
		if (W.damage < 5) //this thing's edges are not in day's travel of each other, what healing?
			heal_amt += 0.01
...
/datum/organ/external/proc/fracture()
	if(status & ORGAN_BROKEN)
		return
	owner.visible_message("\red You hear a loud cracking sound coming from \the [owner].","\red <b>You feel something shatter the bones in your [display_name]!</b>","You hear a sickening crack.")
