...
/datum/game_mode/anti_revolution/proc/add_head_objectives(datum/mind/head)
	...
	for(var/datum/mind/target in demote_targets)
		...
		obj.owner = head
		obj.target = target
		obj.explanation_text = "[target.current.real_name], the [target.assigned_role]  has been classified as harmful to NanoTrasen's goals. Demote them."
		head.objectives += obj
...
/datum/game_mode/anti_revolution/proc/check_crew_victory()
	for(var/datum/mind/head_mind in heads)
		...
		if((head_mind) && (head_mind.current) && (head_mind.current.stat != 2) && T && (T.z <= 4) && !head_mind.is_brigged(600))
