...
/obj/machinery/cellshower/proc/spray()
	...
	for(var/turf/T in range(1, locate(x, y, z + 1)))
		if(T.density)
			continue
		spawn(0)
			var/obj/effect/decal/chempuff/D = new(locate(x, y, z + 1))
			D.create_reagents(5)
...
/obj/effect/shower/HasEntered(atom/movable/O)
	..()
	wash(O)
	if(ismob(O))
		mobpresent += 1
...
/obj/effect/shower/Uncrossed(atom/movable/O)
	if(ismob(O))
		mobpresent -= 1
...
/obj/effect/shower/process()
	if(!master.on || !mobpresent) return
